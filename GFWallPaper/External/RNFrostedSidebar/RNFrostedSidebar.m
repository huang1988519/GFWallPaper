...
- (void)layoutSubviews {
    ...
    self.contentLabel.frame = CGRectMake(0, 0, inset *2, inset *2);
    self.imageView.center = CGPointMake(inset, inset);
    self.contentLabel.center = CGPointMake(inset, inset);
}
...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors {
    if (self = [super init]) {
        ...
        [_images enumerateObjectsUsingBlock:^(id image, NSUInteger idx, BOOL *stop) {
